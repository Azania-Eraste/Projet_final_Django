...
from .models import Avis, Commune, Mode, Produit, TypePaiement, VariationProduit
...
class CheckForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        # pop 'user' if provided (not used here) to avoid passing it to parent
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            ...
            if not Mode.objects.filter(
                utilisateur=user, type_paiement=TypePaiement.LIQUIDE.value, statut=True
            ).exists():
    ...
    def clean(self):
        ...
        if raw_produit_id and "produit_id" not in cleaned_data:
            try:
                ...
                self.add_error(
                    "produit_id", "L'ID du produit doit être un nombre valide."
                )
...
class ModePaiementForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "description": forms.Textarea(
                attrs={"class": "form-control mb-3", "rows": 2}
            ),
            "numero_tel": forms.TextInput(
                attrs={
                    "class": "form-control mb-3",
            ...
            "numero": forms.TextInput(
                attrs={"class": "form-control mb-3", "placeholder": "Numéro de carte"}
            ),
            "expiration": forms.TextInput(
                attrs={"class": "form-control mb-3", "placeholder": "MM/AA"}
            ),
            "code": forms.TextInput(
                attrs={"class": "form-control mb-3", "placeholder": "CVC ou code"}
            ),
        }
    ...
    def clean(self):
        ...
        if nom in ["Wave", "Orange Money", "MTN Money", "Moov Money"]:
            ...
            if not numero_tel:
                self.add_error(
                    "numero_tel", "Le numéro de téléphone est requis pour ce mode."
                )
